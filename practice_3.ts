...
// State : Node들의 공유 보관소
import { MessagesZodState, StateGraph, START, END, interrupt } from "@langchain/langgraph";
...
// interrupt 기능이 있는 Node를 추가합니다
async function authNode(state: z.infer<typeof MessagesZodState>) {
  const lastMessage = state.messages[state.messages.length - 1] as AIMessage;

  if (lastMessage?.tool_calls && lastMessage?.tool_calls?.length > 0) {
    const answer = interrupt(`도구 호출 승인 대기 중: ${JSON.stringify(lastMessage.tool_calls)}`);
...
// agent가 tools가 필요하다고 판단하면 무조건 auth node를 거치도록 합니다
